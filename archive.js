function postDate({ innerText: date }) {
  ...
  date = date
    .replace(/ene\.|sty|янв\.|gen/, 'January')
    .replace(/févr\.|lut|февр\./, 'February')
    .replace(/мар\./, 'March')
    .replace(/abr\.|avr\.|kwi|апр\./, 'April')
    .replace(/ma[ijg]|мая/, 'May')
    .replace(/juin|cze|июн.|giu/, 'June')
    .replace(/juil\.|lip|июл\.|lug/, 'July')
    .replace(/ago\.|août|sie|авг\.|ago/, 'August')
    .replace(/wrz|сент\.|set/, 'September')
    .replace(/paź|окт\.|ott/, 'October')
